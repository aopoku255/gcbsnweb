...
const Header = () => {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 bg-white">
      <nav
        aria-label="Global"
        className="flex items-center justify-between p-6 lg:px-8 mx-auto max-w-7xl px-6"
      >
        <div className="flex lg:flex-1">
          <a href="#" className="-m-1.5 p-1.5">
            <span className="sr-only">GCBSN</span>
        ...
        <div className="flex lg:hidden">
          ...
          >
            ...
            <Bars3Icon aria-hidden="true" className="size-6" color="#680000" />
          </button>
        </div>
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <Link
              key={item.name}
              to={item.href}
              className="text-sm/6 font-semibold text-[#680000] hover:text-gray-900"
            >
              {item.name}
            </Link>
          ))}
              ...
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-[#680000] hover:bg-[#680000] hover:text-white"
                  >
                    {item.name}
                  </a>
                ))}
